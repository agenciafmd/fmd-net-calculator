...
public class FunctionRegistry(bool caseSensitive) : IFunctionRegistry
{
    private const string DynamicFuncName = "Fmd.Net.Calculator.DynamicFunc";
    ...
    public void RegisterFunction(string functionName, Delegate function, bool isIdempotent, bool isOverWritable)
    {
        ...
        if (funcType.FullName.StartsWith("System.Func"))
        {
            foreach (Type genericArgument in funcType.GenericTypeArguments)
                if (genericArgument != typeof(decimal))
                    throw new ArgumentException("Only decimals are supported as function arguments.", "function");

            numberOfParameters = function
                .GetMethodInfo()
                .GetParameters()
                .Count(p => p.ParameterType == typeof(decimal));
        }
        ...
        FunctionInfo functionInfo = new FunctionInfo(functionName, numberOfParameters, isIdempotent, isOverWritable,
