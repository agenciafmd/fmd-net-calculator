...
public class MemoryCache<TKey, TValue>
{
    ...
    private long _counter; // We cannot use DateTime.Now, because the precission is not high enough.

    private readonly ConcurrentDictionary<TKey, CacheItem> _dictionary;
    ...
    public MemoryCache(int maximumSize, int reductionSize)
    {
        ...
        this._reductionSize = reductionSize;

        this._dictionary = new ConcurrentDictionary<TKey, CacheItem>();
    }
    ...
    public TValue this[TKey key]
    {
        get
        {
            CacheItem cacheItem = _dictionary[key];
            cacheItem.Accessed();
    ...
    public TValue GetOrAdd(TKey key, Func<TKey, TValue> valueFactory)
    {
        ...
        CacheItem cacheItem = _dictionary.GetOrAdd(key, k => 
        {
    ...
    {
        if (_dictionary.Count >= _maximumSize) // >= because we want to add an item after this method
        {
            IList<TKey> keysToDelete = (from p in _dictionary.ToArray()
            ...
            {
                CacheItem cacheItem;
                _dictionary.TryRemove(key, out cacheItem);
            }
        }
    }
    ...
    {
        private MemoryCache<TKey, TValue> _cache;
